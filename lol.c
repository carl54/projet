...
Uint32 getpixel(int x, int y) {
  ...
  Uint8 *p = (Uint8 *)murDraw->pixels + y*murDraw->pitch + x*murDraw->format->BytesPerPixel;
...
void drawTexture(SDL_Surface *screen, float x, float y, SDL_Rect wall){
  int tx = max(fabs(x-floor(x+.5)), fabs(y-floor(y+.5)))*murDraw->w; // x-texcoord
  for (int i=0; i<wall.h; i++) {
    int ty = float(i)/float(wall.h)*murDraw->h;
    Uint32 color = getpixel(tx, ty);
